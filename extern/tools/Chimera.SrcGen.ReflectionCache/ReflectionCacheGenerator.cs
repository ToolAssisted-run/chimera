namespace Chimera.SrcGen.ReflectionCache;
...
public sealed class ReflectionCacheGenerator : IIncrementalGenerator
{
		...
		((AnalyzerConfigOptionsProvider Config, ParseOptions ParseOptions) More, Compilation Compilation) providers)
	{
		if (!providers.More.Config.GlobalOptions.TryGetValue("build_property.RootNamespace", out var nSpace)
			...
			|| providers.Compilation.GetTypeByMetadataName("Chimera.Common.StringExtensions.StringExtensions") is null) // project does not have Chimera.Common dependency TODO revisit w/ codegen'd kitchen sink
		{
			return;
		}
...
{(nSpace == "Chimera.Common" ? string.Empty : "\nusing Chimera.Common;")}
