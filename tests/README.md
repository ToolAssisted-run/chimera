...
Correctness gate for the miniHawk project (see [design-principles.md](../docs/design-principles.md)).
...
tester (each found the hard way; see docs/design-principles.md "Phase 0 discoveries"):
...
Both drivers also take `--quick` / `-Quick`: a six-test smoke subset (~1 min
wall clock) covering distinct mappers, both input paths, and a sync-settings
config variant - for CI smoke checks and fast dev iteration. It is NOT a
substitute for the full gate: commits still require the full 26/26 in both
modes.
...
31 tests total; exclusions and rationale are listed in docs/design-principles.md (mapper 5
