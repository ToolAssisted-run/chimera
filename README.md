...
To run: `build\Chimera.exe` on Windows, `build/ChimeraMono.sh` on Linux. Build a core package first (for quickerNES, run its `chimera/build-package.sh` from a sibling checkout; it installs `quickernes.chimeraCore` into `build/Cores/`), then `File > New Project...` and pick that core. A plain rom with no project is `--core=<path> <rom>` from the commandline.
