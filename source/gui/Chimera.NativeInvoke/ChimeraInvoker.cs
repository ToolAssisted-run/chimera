...
{
	public static class ChimeraInvoker
	{
		...
		static ChimeraInvoker()
		{
			var aname = new AssemblyName("NativeInvokeProxyAssembly");
			ImplAssemblyBuilder = AssemblyBuilder.DefineDynamicAssembly(aname, AssemblyBuilderAccess.Run);
			ImplModuleBuilder = ImplAssemblyBuilder.DefineDynamicModule("ChimeraInvokerModule");
			ClassFieldOffset = ChimeraInvokerUtilities.ComputeClassFirstFieldOffset();
			StringOffset = ChimeraInvokerUtilities.ComputeStringOffset();
			ValueArrayElementOffset = ChimeraInvokerUtilities.ComputeValueArrayElementOffset();
		}
		...
		/// <exception cref="InvalidOperationException"><see cref="GetInvoker{T}(Chimera.Common.IImportResolver,Chimera.Common.IMonitor,Chimera.NativeInvoke.ICallingConventionAdapter)"/> overload previously called with <paramref name="dll"/></exception>
		...
		private static InvokerImpl CreateProxy(Type baseType, bool monitor, bool nonTrivialAdapter)
		{
			...
			var baseMethods = baseType.GetMethods(BindingFlags.Instance | BindingFlags.Public)
				.Select(static m => (Info: m, Attr: m.GetCustomAttributes(true).OfType<ChimeraImportAttribute>().FirstOrDefault()))
				.Where(a => a.Attr != null)
				.ToList();

			if (baseMethods.Count == 0)
			{
				throw new InvalidOperationException("Couldn't find any [ChimeraImport] methods to proxy");
			}
			...
			var type = ImplModuleBuilder.DefineType($"Chimera.NativeInvokeProxy{baseType.Name}", TypeAttributes.Class | TypeAttributes.Public | TypeAttributes.Sealed, baseType);
			...
			bool nonTrivialAdapter)
		{
			...
			var delegateType = NativeInvokeUtilities.CreateDelegateType(baseMethod, nativeCall, type, out var delegateInvoke);
	...
	/// <summary>Indicates that an abstract method is to be proxied by ChimeraInvoker.</summary>
	[AttributeUsage(AttributeTargets.Method)]
	public sealed class ChimeraImportAttribute : Attribute
	{
		...
		public ChimeraImportAttribute(CallingConvention c)
		{
			CallingConvention = c;
		}
	}
