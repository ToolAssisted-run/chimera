...
namespace Chimera.Common
{
	...
	/// Therefore, we define the concept of "binding": the <see cref="ChimeraFile"/> attaches itself to an archive member, which is the file that it will actually be using.<br/>
	...
	/// Strings formatted this way are annotated <see cref="ChimeraFilePathAttribute">[ChimeraFilePath]</see>.
	/// </summary>
	/// <remarks>
	...
	/// </remarks>
	public sealed class ChimeraFile : IDisposable
	{
		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		public static bool PathContainsPipe(string s)
			=> s.ContainsOrdinal('|');

		private readonly List<ChimeraArchiveFileItem>? _archiveItems;

		private Stream? _boundStream;

		private IChimeraArchiveFile? _extractor;
		...
		public IList<ChimeraArchiveFileItem> ArchiveItems => (IsArchive ? _archiveItems : null) ?? throw new InvalidOperationException("Can't get archive items from non-archive");
		...
		/// <summary>Makes a new ChimeraFile based on the provided path.</summary>
		public ChimeraFile([ChimeraFilePath] string path, bool allowArchives = true)
		{
		...
		/// <summary>binds the specified ArchiveItem which you should have gotten by interrogating an archive file</summary>
		public ChimeraFile BindArchiveMember(ChimeraArchiveFileItem item) => BindArchiveMember(item.Index);

		/// <summary>binds the selected archive index</summary>
		/// <exception cref="InvalidOperationException">stream already bound</exception>
		public ChimeraFile BindArchiveMember(int index)
		{
			...
			Util.DebugWriteLine($"{nameof(ChimeraFile)} bound {CanonicalFullPath}");
		...
		public ChimeraFile? BindArchiveMember(string? name)
		{
		...
		/// <exception cref="InvalidOperationException">stream already bound</exception>
		private ChimeraFile BindByExtensionCore(IReadOnlyCollection<string> extensions, bool onlyBindSingle = false)
		{
			if (!_rootExists) return this;
			if (_boundStream != null) throw new InvalidOperationException("stream already bound!");
		...
		public ChimeraFile BindFirst() => BindByExtensionCore(Array.Empty<string>());
		...
		public ChimeraFile BindFirstOf(IReadOnlyCollection<string> extensions) => BindByExtensionCore(extensions);
		...
		/// <remarks>You probably should use <see cref="BindSoleItemOf"/> or the archive chooser instead.</remarks>
		public ChimeraFile BindFirstOf(string extension) => BindByExtensionCore(new[] { extension });
		...
		/// <param name="extensions">File extensions; include the leading period in each, and use lowercase.</param>
		public ChimeraFile BindSoleItemOf(IReadOnlyCollection<string> extensions) => BindByExtensionCore(extensions, onlyBindSingle: true);
		...
		public ChimeraArchiveFileItem? FindArchiveMember(string? name)
		...
		public Stream GetStream() => _boundStream ?? throw new InvalidOperationException($"{nameof(ChimeraFile)}: Can't call {nameof(GetStream)}() before you've successfully bound something!");
		...
		public static IFileDearchivalMethod<IChimeraArchiveFile>? DearchivalMethod { get; set; }

		[return: ChimeraFilePath]
		...
		public static (string ArchivePath, string MemberPath)? SplitArchiveMemberPath([ChimeraFilePath] string path)
		{
			var i = path.LastIndexOf('|');
#if DEBUG
			if (path.IndexOf('|') != i) Console.WriteLine($"{nameof(ChimeraFile)} path contains multiple '|'");
#endif
