...
if (ps -C "mono" -o "cmd" --no-headers | grep -Fq "Chimera.exe"); then
	printf "(it seems Chimera is already running, NOT capturing output)\n" >&2
	exec mono Chimera.exe "$@"
fi
o="$(mktemp -u)"
e="$(mktemp -u)"
mkfifo "$o" "$e"
printf "(capturing output in %s/ChimeraMono_last*.txt)\n" "$PWD" >&2
tee ChimeraMono_laststdout.txt <"$o" &
tee ChimeraMono_laststderr.txt <"$e" | sed "s/.*/$(tput setaf 1)&$(tput sgr0)/" >&2 &
exec mono Chimera.exe "$@" >"$o" 2>"$e"
