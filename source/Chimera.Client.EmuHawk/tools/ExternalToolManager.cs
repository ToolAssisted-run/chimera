...
#pragma warning restore RS0030
		...
		private ToolStripMenuItem/*?*/ GenerateToolTipFromFileName(string fileName)
		{
			...
			try
			{
				...
				if (!externalToolFile.GetReferencedAssemblies().Any(static name
					=> name.Name.StartsWithOrdinal("Chimera.") || name.Name.StartsWithOrdinal("BizHawk."))) // pre-rename tools resolve via the legacy assembly map
				{
					return Fail("it doesn't reference a Chimera assembly");
				}
