...
namespace Chimera.Client.EmuHawk
{
	public partial class CoreFeatureAnalysis : ToolFormBase, IToolFormAutoConfig
	{
		...
		private TreeNode CreateCoreTree(CoreInfo ci)
		{
			...
			foreach (var service in ReflectionCache_Chi_Emu_Com.Types.Where(t => t.IsInterface
		...
		private void DoAllCoresTree(CoreInfo current_ci)
		{
			...
			var possibleCoreTypes = CoreRegistry.Instance.AllFactories
				.Select(f => (f.CoreName, Released: CoreRegistry.AttributesFor(f)?.Released ?? false))
				.OrderByDescending(static core => core.Released)
				.ThenBy(static core => core.CoreName)
				.ToList();
			...
			{
				if (!KnownCores.ContainsKey(core.CoreName))
				{
					string img = "Unknown";
					var coreNode = new TreeNode
					{
						Text = core.CoreName + (core.Released ? "" : " (UNRELEASED)"),
						ForeColor = core.Released ? Color.Black : Color.DarkGray,
						ImageKey = img,
						SelectedImageKey = img,
						StateImageKey = img,
					};
