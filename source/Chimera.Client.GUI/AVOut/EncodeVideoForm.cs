...
namespace Chimera.Client.GUI
{
	...
	public sealed class EncodeVideoForm : FormBase
	{
		...
		private readonly FFmpegWriterForm.FormatPreset[] _presets = FFmpegWriterForm.FormatPreset.GetCannedPresets();

		/// <summary>set while the code is filling the command box, so it is not mistaken for typing</summary>
			...
			Func<string, string?> pickOutputFile)
		{
			...
			SuspendLayout();
			ClientSize = new(UIHelper.ScaleX(580), UIHelper.ScaleY(400));
			MinimumSize = new(UIHelper.ScaleX(520), UIHelper.ScaleY(400));
			StartPosition = FormStartPosition.CenterParent;
			ShowIcon = false;
			MaximizeBox = false;

			Controls.Add(MakeHeading("Reproduce a run into a video file. It happens here, while this window is open."));

			// ---- where it goes ---------------------------------------------------
			Controls.Add(MakeLabel("Output file:", 8, 44));
			_output = new TextBox
			{
				Anchor = AnchorStyles.Top | AnchorStyles.Left | AnchorStyles.Right,
				Location = Pt(100, 40),
				Text = suggestedOutputPath,
				Width = UIHelper.ScaleX(388),
			};
			...
			// ---- what writes it --------------------------------------------------
			Controls.Add(MakeLabel("Format:", 8, 76));
			_format = new ComboBox
			{
				DropDownStyle = ComboBoxStyle.DropDownList,
				Location = Pt(100, 72),
				Width = UIHelper.ScaleX(220),
			};
			foreach (var preset in _presets) _format.Items.Add(preset.Name);
			_format.Items.Add(CustomEntry);
			_format.SelectedIndexChanged += (_, _) => PinFormat();
			Controls.Add(_format);

			Controls.Add(MakeLabel("ffmpeg:", 8, 108));
			_command = new TextBox
			{
				Anchor = AnchorStyles.Top | AnchorStyles.Left | AnchorStyles.Right,
				Location = Pt(100, 104),
				Width = UIHelper.ScaleX(472),
			};
			_command.TextChanged += (_, _) =>
			{
				if (_settingCommand) return;
				// typing over a canned command makes it yours
				_format.SelectedIndex = _format.Items.Count - 1;
				UpdateSuggestedExtension();
			};
			Controls.Add(_command);

			// ---- which frames ----------------------------------------------------
			Controls.Add(MakeLabel("From marker:", 8, 144));
			_from = new ComboBox
			{
				DropDownStyle = ComboBoxStyle.DropDownList,
				Location = Pt(100, 140),
				Width = UIHelper.ScaleX(200),
			};
			Controls.Add(_from);
			Controls.Add(MakeLabel("To marker:", 310, 144));
			_to = new ComboBox
			{
				DropDownStyle = ComboBoxStyle.DropDownList,
				Location = Pt(390, 140),
			...
			_range = new Label
			{
				AutoSize = false,
				Location = Pt(100, 168),
				Size = new(UIHelper.ScaleX(472), UIHelper.ScaleY(18)),
			};
			Controls.Add(_range);
			...
			_width = MakeSize(100, 198);
			_height = MakeSize(172, 198);
			Controls.Add(_width);
			Controls.Add(MakeLabel("x", 156, 202));
			...
			_currentSize = MakeLabel($"now {currentOutputSize.Width}x{currentOutputSize.Height}", 348, 202);
		...
		public string ChosenFormat => (string)(_format.SelectedItem ?? "");
		...
		public void Choose(string? from = null, string? to = null, string? output = null, string? format = null)
		{
			...
			if (format is not null) _format.SelectedItem = format;
		}
		...
		{
			var remembered = _presets.FirstOrDefault(p => p.Name == config.FFmpegFormat);
			if (remembered is not null)
			{
				_format.SelectedItem = remembered.Name;
			}
			else if (config.FFmpegFormat == CustomEntry && !string.IsNullOrWhiteSpace(config.FFmpegCustomCommand))
			{
				// only a command someone actually wrote: the config ships a
				// placeholder in this field, and opening on that would be an
				// invitation to encode nothing
				_format.SelectedIndex = _format.Items.Count - 1;
				SetCommand(config.FFmpegCustomCommand);
			}
			else
			{
				// A file that plays anywhere, for the person who came here to make one
				_format.SelectedItem = _presets.First(static p => p.Name is "MP4").Name;
			}
		...
		private void PinFormat()
		{
			var preset = _presets.FirstOrDefault(p => p.Name == ChosenFormat);
			if (preset is null) return;   // [Custom]: leave what is written alone
			SetCommand(preset.Commandline);
			UpdateSuggestedExtension();
		}
		...
		private void RememberChoices()
		{
			...
			Config.FFmpegFormat = ChosenFormat;
			if (ChosenFormat == CustomEntry) Config.FFmpegCustomCommand = _command.Text;
		}

		private void SetInputsEnabled(bool enabled)
		{
			foreach (var control in new Control[]
				{ _output, _browse, _format, _command, _from, _to, _resize, _audioSync, _captureOsd, _captureLua })
			{
