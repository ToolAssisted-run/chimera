	...
	public partial class PlayMovie : Form, IDialogParent
	{
		...
		private void Run()
		{
			...
			if (indices.Count > 0) // Import file if necessary
			{
				...
				if (movie == null) throw new Exception($"Unable to load movie {_movieList[MovieView.SelectedIndices[0]].Filename}");
		...
		{
			using var file = new ChimeraFile(filename);
			if (!file.Exists)
			{
				return null;
			}
		...
		private IBasicMovieInfo LoadMovieInfo(ChimeraFile hf, bool force)
		{
